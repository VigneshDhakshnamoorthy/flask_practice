...
def index() -> str:
    ...
    if request.method == "GET":
        project_select = (
            ...
            if "select_project" in session
            else project_list[0]
        )
        week_select = (
            session["select_week"] if "select_week" in session else week_list[0]
        )
    ...
    activity_this_week: list[tuple] = db_helper.fetch_all(
        "SELECT id, activity, count FROM activity_this_week WHERE project = ? AND week = ?",
        (project_select, week_select),
    )
    ...
    return render_template(
        ...
        selected_week=week_select,
        activity_this_week_enable=activity_this_week_enable,
        activity_this_week_data=activity_this_week_data,
    )
...
def save_data() -> str:
    if request.method == "POST":
        ...
        def save_to_table(table_name, data) -> None:
            project = request.json["project"]
            week = request.json["week"]
            if data:
                data_to_save: list[tuple] = [
                    (project, week) + tuple(row) for row in data
                ]
                data_save_non_delete: list[tuple] = [tup[:-1] for tup in data_to_save]
                filtered_data: list[tuple] = [
                    tup for tup in data_save_non_delete if any(tup[:2]) and any(tup[2:])
                ]
                print(filtered_data)
                db_helper.save_data(
                    table_name, db_helper.get_table_dict()[table_name], filtered_data
                )

        save_to_table("utilization_by_task", request.json["utilization_by_task"])
        save_to_table(
            "utilization_by_resource", request.json["utilization_by_resource"]
        )
...
def edit() -> str:
    ...
    utilization_by_task: list[tuple] = db_helper.fetch_all(
        "SELECT id, task_name, hours FROM utilization_by_task WHERE project = ? AND week = ?",
        (project_select, week_select),
    )
    activity_this_week: list[tuple] = db_helper.fetch_all(
        "SELECT id, activity, count FROM activity_this_week WHERE project = ? AND week = ?",
...
def update(table_name: str) -> Response | None:
    if request.method == "POST" and "data" in request.json:
        ...
        for row in updated_data:
            ...
            where_clause: str = "WHERE id = ?"
            values.append(row["id"])
            set_clause_str: str = ", ".join(set_clause)
            update_string: str = (
                f"UPDATE {table_name} SET {set_clause_str} {where_clause}"
            )
...
def delete(table_name: str, id) -> Response:
