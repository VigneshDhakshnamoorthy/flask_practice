...
data: list[list] = [["John", "Doe", 30], ["Jane", "Smith", 25], ["Bob", "Johnson", 35]]


@app.route("/editable", methods=["GET", "POST"])
def editable() -> Response | str:
...
def save_data() -> str:
    if request.method == "POST":
        ...
        def save_to_table(table_name, data) -> None:
            project: str = request.json["project"]
            week: str = request.json["week"]
            data_to_save: list[tuple] = [(project, week) + tuple(row) for row in data]
            db_helper.save_data(
                table_name, db_helper.get_table_dict()[table_name], data_to_save
            )
...
def fetch_data() -> Response:
    ...
    for table_name in db_helper.get_table_names():
        fetch_data_query: list[tuple] = db_helper.fetch_data(
            table_name, "project=? AND week=?", (project, week)
        )
...
def update_data() -> str:
    ...
    def update_table(table_name, data) -> None:
        for row in data:
            if "id" in row:
                id: int = row.pop("id")
                set_clause: str = ", ".join([f"{key}=?" for key in row.keys()])
                db_helper.update_data(
                    table_name, set_clause, "id=?", tuple(row.values()) + (id,)
                )
